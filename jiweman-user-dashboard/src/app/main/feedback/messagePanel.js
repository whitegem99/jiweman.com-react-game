...
function MessageSidePanel(props) {
	...
	function onMessageSubmit(ev) {
		...
		axios
			.post('/feedback/'+messageData._id, collectionRequestBody)
			.then(response => {
		...
		>
			{messageData && messageData.messages ? (
				...
				<div className={clsx('flex flex-col relative', props.className)}>
					...
					{messageData && messageData.messages && messageData.status !== "Closed" ? (
						<form onSubmit={onMessageSubmit} className="absolute bottom-0 right-0 left-0 py-16 px-8">
							...
							<Typography className="px-16 pb-24 text-center">
							This feedback ticket has been closed.
						</Typography>
