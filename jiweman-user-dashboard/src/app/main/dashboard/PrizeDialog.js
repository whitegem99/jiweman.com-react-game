						...
						<table className="simple clickable">
							<thead>
								<tr>
									...
									{leagueData.gameRegionType === 'international' && <th className="text-right">Prize Amount</th>}
									<th className="text-right">{`Prize Amount(in ${leagueData.localCurrency})`}</th>
								</tr>
							</thead>
							<tbody>
								{leagueData.prize.length ? (
									leagueData.prize.map((row, key) => (
										<tr key={key}>
											...
											{leagueData.gameRegionType === 'international' && <td className="text-right">${row}</td>}
											<td className="text-right">
												{leagueData.prizeInLocalCurrency[key] + ' '}
												{leagueData.localCurrency}
											</td>
										</tr>
									))
								) : (
										<tr>
											<td>No Prize</td>
											{leagueData.gameRegionType === 'international' && <td></td>}
											<td></td>
										</tr>
									)}
							</tbody>
