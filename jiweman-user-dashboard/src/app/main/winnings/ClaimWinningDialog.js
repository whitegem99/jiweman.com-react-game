...
function WinningDialog(props) {
	...
	const winningDialog = useSelector(({ winningsPage }) => {
		// console.log(winningsPage);
		return winningsPage.winnings.winningDialog;
	});
	...
	const requestPayment = async () => {
		...
		const reqBody = {
			phoneNumber,
			winningId: winningDialog.data._id,
			first_name,
			last_name
		};
	...
	const initDialog = useCallback(() => {
		 ...
		 * Dialog type: 'new'
		 */
		if (winningDialog.type === 'new') {
			handleReset();
			getPhoneNumberList(winningDialog.data.league._id);

			console.log(winningDialog);
		}
		// eslint-disable-next-line
	}, [winningDialog.data, winningDialog.type]);

	useEffect(() => {
		/**
		 * After Dialog Open
		 */
		if (winningDialog.props.open) {
			initDialog();
		}
	}, [winningDialog.props.open, initDialog]);
